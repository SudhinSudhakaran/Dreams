...
const DreamsList = () => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('🚀 ~ DreamsList ~ item:', item);
  ...
  const fetchData = async (pageNumber, showLoader) => {
    ...
    let url = `${ApiConnections.DREAMS_LIST}?language=${_l}&page=${pageNumber}`;
    if (item?.charId) {
      url = `${ApiConnections.DREAMS_LIST}?language=${_l}&page=${pageNumber}&${ApiConnections.KEYS.CHAR_ID}=${item?.charId}`;
    }

    const [isSuccess, message, response] = await get(url);
